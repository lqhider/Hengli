...
jQuery.fn.pagination = function(maxentries, opts) {
	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			if (opts.isJump) {
				// Generate Jump Input
				panel.append(jQuery("<span class='jump_text'>跳转到：</span>"));
				panel.append(jQuery("<input type='text'  style='text-align:center;font-weight:bold' id='jump-index' title='请输入正整数' size='1' />"));
				$("#jump-index").on("input propertychange",function(){
					var v=$(this).val();
					$(this).val(v.replace(/\D/g,''));
				})
				// Generate Jump Handler
				var index = null;
				var jump = jQuery("<a class='gogogo'>" + opts.jump_text + "</a>").bind("click", function(evt) {
			...
			// sum total
			if (opts.isSum) {
				panel.append("<a>共&nbsp;" + dataCount + "&nbsp;条记录</a>");
			}
